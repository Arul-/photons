...
/**
 * Gemini Runner — executes Gemini CLI agents locally per group folder.
 *
 * Spawns `gemini -p` as a subprocess with the group folder as cwd.
 * Injects conversation memory from previous runs since Gemini CLI
 * has no native session continuity. Manages concurrency so multiple
 * groups don't overwhelm the system.
 *
 * @version 1.0.0
 * @icon ♊
 * @tags agent, gemini, runner
 * @stateful
 */
export default class GeminiRunner extends Photon {
  ...
  protected settings = {
    ...
    maxConcurrent: 3,
    /** Gemini model to use (empty = CLI default) */
    model: '',
  ...
  /**
   * Run a prompt against a group's context using Gemini CLI.
   * Returns the agent's text response.
   *
   * @title Run Agent
   * @openWorld
   * @param groupFolder Group folder name {@example "dev-team"}
   * @param prompt The prompt to send to Gemini {@example "Summarise the discussion"}
  ...
  private async _execute(
    ...
    try {
      // Always inject memory — Gemini has no native session continuity
      const memCtx = await this._buildMemoryContext(groupFolder, prompt);
      const promptWithMemory = memCtx ? `${memCtx}\n\n${prompt}` : prompt;

      const result = await this._spawnGemini(groupDir, groupFolder, promptWithMemory, systemPrompt, runState);
  ...
  private _spawnGemini(
    ...
    return new Promise((resolve) => {
      ...
      if (this.settings.sandbox && this.settings.sandbox !== 'none') {
        args.push('--sandbox', this.settings.sandbox);
      }
