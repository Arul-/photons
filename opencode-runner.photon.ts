...
/**
 * OpenCode Runner — executes OpenCode AI coding agents per group folder.
 *
 * Spawns `opencode` as a subprocess in the group folder. Injects
 * conversation memory from previous runs since OpenCode has no native
 * session continuity. Manages concurrency across groups.
 *
 * @version 1.0.0
 * @icon 🌐
 * @tags agent, opencode, runner
 * @stateful
 */
export default class OpencodeRunner extends Photon {
  ...
  /**
   * Run a prompt against a group's context using OpenCode.
   * Returns the agent's text response.
   *
   * @title Run Agent
   * @openWorld
   * @param groupFolder Group folder name {@example "dev-team"}
   * @param prompt The prompt to send to OpenCode {@example "Fix the failing tests"}
  ...
  private async _execute(
    ...
    try {
      // Always inject memory — opencode has no native session continuity
      const memCtx = await this._buildMemoryContext(groupFolder, prompt);
      const promptWithMemory = memCtx ? `${memCtx}\n\n${prompt}` : prompt;

      const result = await this._spawnOpencode(groupDir, groupFolder, promptWithMemory, systemPrompt, runState);
  ...
  private _spawnOpencode(
    ...
    return new Promise((resolve) => {
      ...
      // -q suppresses spinner/progress output
